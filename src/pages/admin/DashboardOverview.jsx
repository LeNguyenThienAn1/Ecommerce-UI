...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
                // === Products ===
                const resProducts = await fetch("https://localhost:7165/api/products/paging", {
                  method: "POST",
                  headers: {
                ...
                let currentTotalProducts = 0; // Renamed to avoid confusion with setStats
                let productsData = [];
                        try {
                          console.log("Raw products response:", textProducts); // Log raw response
                          const parsed = JSON.parse(textProducts);
                          console.log("Parsed products object:", parsed); // Log parsed object
                          // Assuming the paginated response has a 'totalCount' property
                          // or an 'items' array, otherwise fall back to the root array or $values
                          productsData = Array.isArray(parsed) ? parsed : (parsed.items || parsed.$values || []);
                          currentTotalProducts = parsed.totalCount || productsData.length;
                        } catch (err) {                  console.error("Parse products error:", err, textProducts);
                }
                ...
                try {
                  ...
                  categoriesData = Array.isArray(parsed) ? parsed : (parsed.items || parsed.$values || []);
                  currentTotalCategories = parsed.totalCount || categoriesData.length;
                } catch (err) {
                ...
                try {
                  ...
                  usersData = Array.isArray(parsed) ? parsed : (parsed.items || parsed.$values || []);
                  currentTotalUsers = parsed.totalCount || usersData.length;
                } catch (err) {
                ...
                try {
                  ...
                  ordersData = Array.isArray(parsed) ? parsed : (parsed.items || parsed.$values || []);
                  currentTotalOrders = parsed.totalCount || ordersData.length;
                  
                  // Calculate total revenue from successful/paid orders
                  if (Array.isArray(ordersData)) {
                    totalRevenue = ordersData.reduce((acc, order) => {
                      // Status 5: Successfully, Status 6: Paid
            ...
            <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
                  ...
                  <p className="text-3xl font-bold mb-2">{stats.totalRevenue.toLocaleString("en-US")} ₫</p>
                  <div className="flex items-center gap-2 text-sm">
                    <TrendingUp className="w-4 h-4" />
                    <span>Based on successful orders</span>
                  </div>
                </div>
        
                {/* Quick Stats */}
                <div className="bg-white rounded-xl shadow-sm p-6">          <h3 className="text-lg font-semibold text-gray-800 mb-4">Quick Stats</h3>
